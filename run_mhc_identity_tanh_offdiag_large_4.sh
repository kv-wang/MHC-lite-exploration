...
#   WANDB_LOG_H_MATRIX_GRAD_NORM=True H_RES_GRAD_DUMP_INTERVAL=500 ./run_mhc_identity_tanh_offdiag_large_4.sh
...
WANDB_PROJECT="${WANDB_PROJECT:-final_experiments_large}"
STREAMS_LIST="${STREAMS_LIST:-16 32}"
...
WANDB_LOG_H_MATRIX_GRAD_NORM="${WANDB_LOG_H_MATRIX_GRAD_NORM:-False}"
H_RES_GRAD_DUMP_INTERVAL="${H_RES_GRAD_DUMP_INTERVAL:-0}"
H_RES_GRAD_DUMP_DIR="${H_RES_GRAD_DUMP_DIR:-}"
MHC_LOG_H_RES_GAMMA="${MHC_LOG_H_RES_GAMMA:-False}"
...
run_streams() {
  ...
  echo ""
  echo "================================================================"
  echo " Running large mHC identity-tanh-offdiag with ${n_streams} streams"
  echo " wandb_project:     $WANDB_PROJECT"
