...
while True:
    ...
    if master_process:
        for name, module in model.named_modules():
            if not hasattr(module, "I_AM_ANALYSIS_BLOCK"):
                continue
            module.clear_info()
            module.log_info = True
    ...
    if master_process:
        for name, module in model.named_modules():
            if not hasattr(module, "I_AM_ANALYSIS_BLOCK"):
                continue
            if iter_num not in logged_infos:
                logged_infos[iter_num] = {}
            logged_infos[iter_num][name] = copy.deepcopy(module.info)

            module.clear_info()
            module.log_info = False
...
if master_process:
    print ("Saving logged infos")
    save_dir = "log_out/"
    os.makedirs(save_dir, exist_ok=True)
    with open(os.path.join(save_dir, analysis_filename), 'wb') as f:
        pickle.dump(logged_infos, f)
